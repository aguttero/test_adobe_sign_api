...
class User(Base):
    ...
    last_sync: Mapped[dt.date] = mapped_column(
        Date, default=dt.datetime.today, onupdate=dt.datetime.today, nullable=False
    )
    ...
    def __repr__(self) -> str:
        return (
            f"User(id={self.id!r}, email={self.email!r}, last_name={self.last_name!r})"
        )
...
class Agreement(Base):
    ...
    jad_content: Mapped["JadContent"] = relationship(
        back_populates="agreement", cascade="all, delete-orphan"
    )
...
class JadContent(Base):
    ...
    qa_flag: Mapped[Optional[bool]]
    rep_notes: Mapped[Optional[str]]
    dev_notes: Mapped[Optional[str]]
    last_sync: Mapped[dt.date] = mapped_column(
        Date,
        default=dt.datetime.today,
        onupdate=dt.datetime.today,
...
class Document(Base):
    ...
    file_status: Mapped[str] = mapped_column(
        default="pending"
    )  # lifecycle status for pdf and txt
    # file_status: Mapped[str] = mapped_column(
    #     str,
...
def parse_agreements_v1(
    api_agreement_data: List[dict], group_pk_lookup: dict[str, int]
) -> List[Agreement]:
    ...
    for item in api_agreement_data:
        ...
        if group_id_ref is None:
            logger.warning(
                f"Skipping agreement {item.get('id')} — unknown adobe_group_id"
            )
            continue
        ...
        if doc_field_contents_data:
            for field in doc_field_contents_data:
                doc_field_contents.append(
                    DocFieldContent(
                        agreement_id=item.get(
                            "id"
                        ),  # This will be the Agreement's ID after it's created
                        agreement_subtype=field.get("subType"),
                        requester_area=field.get(
                            "defaultValue", ""
                        ),  # Assuming defaultValue contains relevant info
                    )
                )
